...
import { getUserByEmail } from '@/lib/supabaseStorage';
import bcrypt from 'bcryptjs';

// Simple in-memory rate limiting (use MongoDB/database for persistence in production)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // user._id is guaranteed to exist because we checked `if (!user)` above
...
// Explicitly set runtime to Node.js if needed to avoid Edge Runtime conflicts
// Uncomment the line below if you encounter issues related to Edge Runtime
